...
/**
 * Example of using Cloud IoT device manager API to administer devices,
 * registries and projects.
 *
 * <p>
 * This example uses the Device Manager API to create, retrieve, disable, list
 * and delete Cloud IoT devices and registries, using both RSA and eliptic curve
 * keys for authentication.
 *
 * <p>
 * To start, follow the instructions on the Developer Guide at
 * cloud.google.com/iot to create a service_account.json file and Cloud Pub/Sub
 * topic as discussed in the guide. You will then need to point to the
 * service_account.json file as described in
 * https://developers.google.com/identity/protocols/application-default-credentials#howtheywork
 *
 * <p>
 * Before running the example, we have to create private and public keys, as
 * described in cloud.google.com/iot. Since we are interacting with the device
 * manager, we will only use the public keys. The private keys are used to sign
 * JWTs to authenticate devices. See the <a href=
 * "https://github.com/GoogleCloudPlatform/java-docs-samples/tree/master/iot/api-client/mqtt_example">MQTT
 ...
 * </pre>
 */
public class DeviceRegistryImpl extends GoogleCloudHelper {
	...
	public static Topic createIotTopic(String projectId, String topicId) throws Exception {
		...
		GoogleCredentials credentials = GoogleCredentials.fromStream(new ByteArrayInputStream(CloudConfig.googleAPICredential.getBytes())).createScoped(CloudIotScopes.all());

		try (TopicAdminClient topicAdminClient = TopicAdminClient.create(TopicAdminSettings.newBuilder().setCredentialsProvider(FixedCredentialsProvider.create(credentials)).build())) {
			...
			Binding binding = Binding.newBuilder().addMembers("serviceAccount:" + CloudConfig.serviceAccount).setRole(Role.owner().toString()).build();
	...
	public static void createRegistry(String cloudRegion, String projectId, String registryName, String pubsubTopicPath) throws GeneralSecurityException, IOException {
		GoogleCredential credential = GoogleCredential.fromStream(new ByteArrayInputStream(CloudConfig.googleAPICredential.getBytes())).createScoped(CloudIotScopes.all()).createScoped(CloudIotScopes.all());
		JsonFactory jsonFactory = JacksonFactory.getDefaultInstance();
		HttpRequestInitializer init = new RetryHttpInitializerWrapper(credential);
		final CloudIot service = new CloudIot.Builder(new NetHttpTransport(), jsonFactory, init).setApplicationName(GoogleCloudHelper.APP_NAME).build();
	...
	public static List<CustomDevice> listDevices(String projectId, String cloudRegion, String registryName) throws Exception {
		final CloudIot service = createService(CloudConfig.googleAPICredential);

		final String registryPath = String.format("projects/%s/locations/%s/registries/%s", projectId, cloudRegion, registryName);

		List<Device> devices = service.projects().locations().registries().devices().list(registryPath).setFieldMask("config,gatewayConfig,metadata,state").execute().getDevices();

		if (devices == null) {
			return new ArrayList<>();
		}

		List<CustomDevice> customDevices = new ArrayList<>();
		for (Device device : devices) {
			customDevices.add(new CustomDevice(device));
		}
	 ...
	 *
	 * <p>
	 * This is a valid way to construct a device, however until it is patched
	 * with a credential the device will not be able to connect to Cloud IoT.
	 */
	public static void createDeviceWithNoAuth(String deviceId, String projectId, String cloudRegion, String registryName) throws GeneralSecurityException, IOException {
		GoogleCredential credential = GoogleCredential.fromStream(new ByteArrayInputStream(CloudConfig.googleAPICredential.getBytes())).createScoped(CloudIotScopes.all()).createScoped(CloudIotScopes.all());
		JsonFactory jsonFactory = JacksonFactory.getDefaultInstance();
		HttpRequestInitializer init = new RetryHttpInitializerWrapper(credential);
		final CloudIot service = new CloudIot.Builder(new NetHttpTransport(), jsonFactory, init).setApplicationName(GoogleCloudHelper.APP_NAME).build();
	...
	public static CustomGateway getGateway(String deviceId, String projectId, String cloudRegion, String registryId) throws GeneralSecurityException, IOException {
		CustomDevice customDevice = getDevice(deviceId, projectId, cloudRegion, registryId);
		if(customDevice == null){
			return null;
		}
		return new CustomGateway(customDevice.getDevice());
	}
	// [END iot_get_device]
	
	// [START iot_get_device]
	/** Retrieves device metadata from a registry. * */
	public static CustomDevice getDevice(String deviceId, String projectId, String cloudRegion, String registryId) throws GeneralSecurityException, IOException {
		GoogleCredential credential = GoogleCredential.fromStream(new ByteArrayInputStream(CloudConfig.googleAPICredential.getBytes())).createScoped(CloudIotScopes.all()).createScoped(CloudIotScopes.all());
		JsonFactory jsonFactory = JacksonFactory.getDefaultInstance();
		HttpRequestInitializer init = new RetryHttpInitializerWrapper(credential);
		final CloudIot service = new CloudIot.Builder(new NetHttpTransport(), jsonFactory, init).setApplicationName(GoogleCloudHelper.APP_NAME).build();

		final String devicePath = String.format("projects/%s/locations/%s/registries/%s/devices/%s", projectId, cloudRegion, registryId, deviceId);
	...
	/** Retrieves device metadata from a registry. * */
	public static List<DeviceState> getDeviceStates(String projectId, String cloudRegion, String registryName, String deviceId) throws GeneralSecurityException, IOException {
		GoogleCredential credential = GoogleCredential.fromStream(new ByteArrayInputStream(CloudConfig.googleAPICredential.getBytes())).createScoped(CloudIotScopes.all()).createScoped(CloudIotScopes.all());
		JsonFactory jsonFactory = JacksonFactory.getDefaultInstance();
		HttpRequestInitializer init = new RetryHttpInitializerWrapper(credential);
		final CloudIot service = new CloudIot.Builder(new NetHttpTransport(), jsonFactory, init).setApplicationName(GoogleCloudHelper.APP_NAME).build();
	...
	public static DeviceState getDeviceLatestStates(String projectId, String cloudRegion, String registryName, String deviceId) throws GeneralSecurityException, IOException {
		GoogleCredential credential = GoogleCredential.fromStream(new ByteArrayInputStream(CloudConfig.googleAPICredential.getBytes())).createScoped(CloudIotScopes.all()).createScoped(CloudIotScopes.all());
		JsonFactory jsonFactory = JacksonFactory.getDefaultInstance();
		HttpRequestInitializer init = new RetryHttpInitializerWrapper(credential);
		final CloudIot service = new CloudIot.Builder(new NetHttpTransport(), jsonFactory, init).setApplicationName(GoogleCloudHelper.APP_NAME).build();
		...
		return resp.getDeviceStates().get(0);
	}
	...
	/**
	 * Set a device configuration to the specified data (string, JSON) and
	 * version (0 for latest).
	 */
	public static void setDeviceConfiguration(String deviceId, String projectId, String cloudRegion, String registryId, String data, long version) throws GeneralSecurityException, IOException {
		GoogleCredential credential = GoogleCredential.fromStream(new ByteArrayInputStream(CloudConfig.googleAPICredential.getBytes())).createScoped(CloudIotScopes.all()).createScoped(CloudIotScopes.all());
		JsonFactory jsonFactory = JacksonFactory.getDefaultInstance();
		HttpRequestInitializer init = new RetryHttpInitializerWrapper(credential);
		final CloudIot service = new CloudIot.Builder(new NetHttpTransport(), jsonFactory, init).setApplicationName(GoogleCloudHelper.APP_NAME).build();
		...
		String encPayload = Base64.encodeBase64String(data.getBytes("UTF-8"));
	...
	// [START send_command]
	public static void sendCommand(String deviceId, String projectId, String cloudRegion, String registryName, String data) throws GeneralSecurityException, IOException {
		GoogleCredential credential = GoogleCredential.fromStream(new ByteArrayInputStream(CloudConfig.googleAPICredential.getBytes())).createScoped(CloudIotScopes.all()).createScoped(CloudIotScopes.all());
		JsonFactory jsonFactory = JacksonFactory.getDefaultInstance();
		HttpRequestInitializer init = new RetryHttpInitializerWrapper(credential);
		final CloudIot service = new CloudIot.Builder(new NetHttpTransport(), jsonFactory, init).setApplicationName(GoogleCloudHelper.APP_NAME).build();
		...
		SendCommandToDeviceResponse res = service.projects().locations().registries().devices().sendCommandToDevice(devicePath, req).execute();

		System.out.println("Command response: " + res.toString());
	}
	// [END send_command]

	public static void bindDeviceToGateway(String projectId, String cloudRegion, String registryName, String deviceId, String gatewayId) throws GeneralSecurityException, IOException {
		...
		BindDeviceToGatewayResponse response = service.projects().locations().registries().bindDeviceToGateway(registryPath, request).execute();

		System.out.println(String.format("Device bound: %s", response.toPrettyString()));
		// [END bind_device_to_gateway]
	}

	public static void unbindDeviceFromGateway(String projectId, String cloudRegion, String registryName, String deviceId, String gatewayId) throws GeneralSecurityException, IOException {
	...
	public static void attachDeviceToGateway(MqttAsyncClient client, String deviceId) throws MqttException {
		// [START attach_device]
	...
	public static void detachDeviceFromGateway(MqttAsyncClient client, String deviceId) throws MqttException {
		// [START detach_device]
	...
	public static void createGateway(String projectId, String cloudRegion, String registryName, String gatewayId, String certificateFilePath, String algorithm) throws GeneralSecurityException, IOException {
	...
	public static List<CustomGateway> listGateways(String projectId, String cloudRegion, String registryName) throws Exception {
		// [START list_gateways]
		final CloudIot service = createService(CloudConfig.googleAPICredential);
		final String registryPath = String.format("projects/%s/locations/%s/registries/%s", projectId, cloudRegion, registryName);
		...
		List<Device> devices = service.projects().locations().registries().devices().list(registryPath).setFieldMask("config,gatewayConfig,metadata,state").execute().getDevices();
	       ...
	       List<CustomGateway> gateways = new ArrayList<CustomGateway>();
	        if (devices == null) {
	            return gateways;
	        }
	        ...
	        for (Device device : devices) {
	            if (device.getGatewayConfig() != null
	                    && device.getGatewayConfig().getGatewayType() != null
	                    && device.getGatewayConfig().getGatewayType().equals("GATEWAY")) {
	                if (device.getConfig() != null) {
	                	gateways.add(new CustomGateway(device));
	                }
	            }
	        }
	        return gateways;
	 ...
	 * @throws Exception
	 */
	public static List<CustomDevice> listDevicesForGateway(String projectId, String cloudRegion, String registryName, String gatewayId) throws Exception {
		// [START list_devices_for_gateway]
		final CloudIot service = createService(CloudConfig.googleAPICredential);

		final String gatewayPath = String.format("projects/%s/locations/%s/registries/%s/devices/%s", projectId, cloudRegion, registryName, gatewayId);

		final String registryPath = String.format("projects/%s/locations/%s/registries/%s", projectId, cloudRegion, registryName);

		List<Device> devices = service.projects().locations().registries().devices().list(registryPath).setFieldMask("config,gatewayConfig,metadata,state").setGatewayListOptionsAssociationsGatewayId(gatewayId).execute().getDevices();

		if (devices == null) {
			return new ArrayList<>();
		}
		ArrayList<CustomDevice> customDevices = new ArrayList<>();
		for (Device device : devices) {
			customDevices.add(new CustomDevice(device));
		}
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		DeviceRegistryOptions options = DeviceRegistryOptions.fromFlags(args);
		if (options == null) {
			// Could not parse.
			return;
		}
		...
		case "send-command":
			...
			sendCommand(options.deviceId, options.projectId, options.cloudRegion, options.registryName, options.commandData);
			break;
		default:
